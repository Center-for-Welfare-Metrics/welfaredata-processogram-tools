...
`--ci` is the leaf-level clickable object — the most granular navigable unit in the
hierarchy. When a user clicks a `--ci` element, the platform opens its description,
scientific context, and related questions.
 
A `--ci` can represent any of the following, depending on the processogram context:
 
| Type | Example |
|---|---|
| Physical component | cage, drinker, conveyor, transport crate |
| Condition or indicator | temperature zone, lighting condition |
| Individually visible animal | `chick--ci-01`, `sow--ci-01` |
| Representative group of animals | `chick_group--ci-01`, `salmon_representative--ci-01` |
| Action scene | chick being sexed, operator handling equipment |
 
The name alone does not define what a `--ci` means — context and hierarchy do.
A `chick--ci-01` inside a `hatchery--ph` means something different from the same
element inside a `transport--ph`. Do not create new semantic classes to encode
context into names (e.g. `crowded_chick--ci` or `stressed_hen--ci`). Use the
hierarchy to carry context.
 
The decision of whether something should be its own `--ci` — or a visual detail
inside a parent group — is a semantic decision owned by the Welfare Footprint
Institute. When in doubt, ask Wladimir.
