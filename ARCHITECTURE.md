...
**Note on Fix 2b (ViewBox Shifting) — discarded:** an on-demand rasterization
approach was investigated as a second quality layer for extreme zoom cases where
even 8× is insufficient. It was discarded because SVG renders all elements within
the viewBox coordinates — not only the focused one. Neighboring elements appeared
in the tile, and isolating a single element would require hiding N–1 elements,
adding 50–200ms delay per zoom step. The adaptive mid tier is the current and
only tier strategy. See Issue #1 for full details.
...
├── AGENTS.md                            AI agent orientation — what agents can and cannot do
├── ARCHITECTURE.md                      ← this file — engine architecture and decisions
│
└── docs/
    ├── documentation-map.md             hierarchy and authority of all documents
    ├── svg-preparation-requirements.md  SVG technical rules — source of truth for designers
    ├── architecture-decisions/
    │   └── adr-001-*.md                 architectural decisions — active, deprecated, pending
    ├── handover/
