...
- **Numeric suffix uses underscore instead of hyphen** — IDs like `pig--ci_01` or `broiler--ci_01` use an underscore before the number instead of a hyphen. The Canvas Navigator only recognizes the hyphen format: `pig--ci-01`, `broiler--ci-01`. IDs with underscore suffixes are invisible to the motor's hit-testing system and will not be clickable. Always use a hyphen before the numeric suffix.
...
- **Stroke-only --ci groups (no fill)** — A `--ci` group contains only shapes with `fill:none` — only strokes are visible. If the stroke is thin, the clickable area may be too narrow for reliable user interaction. Consider adding a transparent fill or increasing the stroke width.
